...
import {
  CreatePedidoRequest,
  createPedidoSchema,
} from "../../validations/pedido_validacao";
import { database } from "../../config/database";
import { PedidoFiltrosInterface } from "../../core/interfaces/pedidoInterface";
import { parseTxtToJson } from "../../config/ficheiro/readFile";
export default class PedidoController {
  ...
  criarPedido = async (
    ...
    const result = await this.pedidoService.criarPedido({
      payload,
  ...
  lerFicheiroText = async (req: Request, res: Response): Promise<Response> => {
    const file = req.file;

    const caminho_arquivo = `../../uploads/${file?.filename}`;
    const readFile = await parseTxtToJson(caminho_arquivo);

    return res.status(201).send(readFile);
  };
}
